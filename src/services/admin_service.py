...
class AdminService:
    ...
    def get_platform_stats() -> Dict[str, Any]:
        ...
        try:
            ...
            active_users = (
                db.session.query(func.count(User.user_id)).filter(User.is_active.is_(True)).scalar()
                or 0
            )
            suspended_users = (
            ...
            # Role distribution
            admins = (
                db.session.query(func.count(User.user_id)).filter(User.role == "admin").scalar()
                or 0
            )
            staff = (
                db.session.query(func.count(User.user_id)).filter(User.role == "staff").scalar()
                or 0
            )
            students = (
                db.session.query(func.count(User.user_id)).filter(User.role == "student").scalar()
                or 0
            )
            ...
            draft_resources = (
                db.session.query(func.count(Resource.resource_id))
                .filter(Resource.status == "draft")
                .scalar()
                or 0
            )
            archived_resources = (
                db.session.query(func.count(Resource.resource_id))
                .filter(Resource.status == "archived")
                .scalar()
                or 0
            )
            ...
            cancelled_bookings = (
                db.session.query(func.count(Booking.booking_id))
                .filter(or_(Booking.status == "cancelled", Booking.status == "rejected"))
                .scalar()
                or 0
            )
            ...
            unread_messages = (
                db.session.query(func.count(Message.message_id))
            ...
            hidden_reviews = (
                ...
                .filter(Review.is_hidden.is_(True))
                .scalar()
                or 0
            )
    ...
    def get_recent_activity(limit: int = 20) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            # Recent users
            recent_users = (
                db.session.query(User).order_by(User.created_at.desc()).limit(limit).all()
            )

            # Recent resources
            recent_resources = (
                db.session.query(Resource).order_by(Resource.created_at.desc()).limit(limit).all()
            )

            # Recent bookings
            recent_bookings = (
                db.session.query(Booking).order_by(Booking.created_at.desc()).limit(limit).all()
            )

            # Recent messages
            recent_messages = (
                db.session.query(Message).order_by(Message.timestamp.desc()).limit(limit).all()
            )

            # Recent reviews
            recent_reviews = (
                db.session.query(Review).order_by(Review.timestamp.desc()).limit(limit).all()
            )
    ...
    def get_user_activity_summary(user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            bookings_count = (
                db.session.query(func.count(Booking.booking_id))
                .filter(Booking.requester_id == user_id)
                .scalar()
                or 0
            )
    ...
    def get_popular_resources(limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = (
                db.session.query(Resource, func.count(Booking.booking_id).label("booking_count"))
                .outerjoin(Booking)
                .filter(Resource.status == "published")
    ...
    def get_booking_trends(days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            trends = (
                ...
                .filter(Booking.created_at >= cutoff_date)
    ...
    @staticmethod
    def process_booking_approvals(
        booking_ids: List[int], action: str, admin_id: int
    ) -> Dict[str, int]:
        """
    ...
    def get_flagged_reviews(limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            reviews = (
                Review.query.filter(Review.is_hidden.is_(True))
