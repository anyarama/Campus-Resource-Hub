...
class QueryParser:
    ...
    # Location patterns  
    LOCATION_PATTERN = r'\b(?:in|at|near|on)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*(?:\s+Hall|\s+Building)?)\b'
    ...
    def _extract_keywords(cls, query: str) -> List[str]:
        ...
        # Remove common words
        stop_words = {'find', 'show', 'me', 'need', 'looking', 'for', 'a', 'an', 'the', 
                      'in', 'at', 'on', 'is', 'are', 'with', 'and', 'or'}
...
class AIConciergeService:
        ...
        try:
            ...
            if 'min_capacity' in filters:
                all_resources = [
                    ...
                    if r.capacity and r.capacity >= filters['min_capacity']
                ]
    ...
    @classmethod
    def _multiple_results_response(cls, resources: List, params: Dict, filters: Dict) -> Dict[str, Any]:
        ...
        if params.get('time') and params.get('date'):
            msg_parts.append(f"I found {count} {category} available {params['date']} {params['time']}:")
        elif params.get('capacity'):
            msg_parts.append(f"I found {count} {category} that can accommodate {params['capacity']} people:")
        else:
    ...
    def _single_result_response(cls, resource, params: Dict) -> Dict[str, Any]:
        ...
        msg = f"I found exactly what you're looking for!\n\n"
